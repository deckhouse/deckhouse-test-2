...
func registrationHandler(input *go_hook.HookInput, dc dependency.Container) error {
	...
	data, err := createMadisonPayload(domainTemplate, getPrometheusURLSchema(prometheusHTTPSMode, globalHTTPSMode))
	if err != nil {
		return err
	}
...
func createMadisonPayload(domainTemplate string, schema string) (madisonRequestData, error) {
